...
export class IntegrationService {
    ...
    if (picture) {
      if (picture?.indexOf('imagedelivery.net') > -1) {
        ...
        try {
          ...
          console.error('Failed to upload integration picture, continuing without it:', error);
  ...
  async saveInstagram(
    ...
    try {
      ...
      console.error('Error updating Instagram integration with picture, retrying without:', error);
      await this._integrationRepository.updateIntegration(id, {
        picture: undefined,
        internalId: getIntegrationInformation.id,
        name: getIntegrationInformation.name,
  ...
  async saveLinkedin(org: string, id: string, page: string) {
    ...
    try {
      ...
      console.error('Error updating LinkedIn integration with picture, retrying without:', error);
      await this._integrationRepository.updateIntegration(String(id), {
        picture: undefined,
        internalId: String(getIntegrationInformation.id),
        name: getIntegrationInformation.name,
  ...
  async saveFacebook(org: string, id: string, page: string) {
    ...
    try {
      ...
      console.error('Error updating Facebook integration with picture, retrying without:', error);
      await this._integrationRepository.updateIntegration(id, {
        picture: undefined,
        internalId: getIntegrationInformation.id,
        name: getIntegrationInformation.name,
